...
async function shortenAndDecorate(fullNews: string): Promise<string> {
  try {
    const prompt = `Take this news content and create a short, engaging summary (3-4 paragraphs max),write in russian. Decorate it with different shrifts relevant emojis to make it fun and visually appealing for a Telegram channel post. Keep it concise and exciting:\n\n${fullNews}`;
    const result = await model.generateContent(prompt);
    const text = typeof result.response.text === "function" ? result.response.text() : result.response.text;
